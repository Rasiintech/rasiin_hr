...
def execute(filters=None):
	...
	for ss in salary_slips:
		...
		row.append(ss.paid_amount)
		row.append(flt(ss.net_pay) - flt(ss.paid_amount))
		row.append(currency or company_currency)  # Fix: You must match this with a column

		frappe.errprint(flt(ss.net_pay) - flt(ss.paid_amount))
		data.append(row)
...
def get_columns(salary_slips):
	...
	columns = (
		...
		+ [
			...
			_("Currency") + ":Link/Currency:80",  # ✅ Fix: Add this to match row
		]
	)

...
def execute(filters=None):
	...
	for ss in salary_slips:
		row = {
			...
			"title": frappe.db.get_value("Employee" ,ss.employee , 'title'),
			"company": ss.company,
		...
		if currency == company_currency:
			atted_d =frappe.db.get_value("Employee" , ss.employee , "amount") - ss.gross_pay
			row.update(
				{
					"attendance_deduction" : atted_d,
					"percent" : ss.salary_descrease,
					"gross_pay": frappe.db.get_value("Employee" , ss.employee , "amount"),
					"total_deduction": flt(atted_d+ss.salary_descrease) * flt(ss.exchange_rate),
			...
			row.update(
				{"gross_pay": frappe.db.get_value("Employee" , ss.employee , "amount"), "total_deduction": ss.total_deduction + ss.salary_descrease, "net_pay": ss.net_pay}
			)
...
def get_columns(earning_types, ded_types):
	...
	# 	columns.append(
	# 		{
	# 			"label": earning_types[0],
	# 			"fieldname": frappe.scrub(earning_types[0]),
	# 			"fieldtype": "Currency",
	# 			"options": "currency",
	# 			"width": 120,
	# 		}
	...
	columns.extend(
		[
			...
			# },
				{
			"label": _("Attendacne Deduction"),
			"fieldname": "attendance_deduction",
			"fieldtype": "Currency",
			"options": "currency",
			"width": 120,
		},
			{
			"label": _("Percent Deduction"),
			"fieldname": "percent",
			"fieldtype": "Currency",
			"options": "currency",
			"width": 120,
